...
from avatar.util import get_code_name, force_bytes, invalidate_cache
...
class Avatar(models.Model):
    contact = models.ForeignKey(Contact, related_name="avatars")
    primary = models.BooleanField(default=False)
    avatar = models.ImageField(
        max_length=1024, upload_to=avatar_file_path, storage=avatar_storage, blank=True)
    ...
    def __unicode__(self):
        return _(six.u('Avatar for %s')) % self.contact

    def save(self, *args, **kwargs):
        avatars = Avatar.objects.filter(contact=self.contact)
        if self.pk:
    ...
    def create_thumbnail(self, size, quality=None):
        ...
        invalidate_cache(self.contact, size)
        try:
